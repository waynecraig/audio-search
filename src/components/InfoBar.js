...
const urlPrefix =
  location.protocol + "//moca-yinchuan.art/audio/admin/?q=service/node/";
...
function intent(input$, http$, direct$) {
  ...
  return xs.merge(
    input$.map((input) => ({
      type:
        cmds[input.payload] || (/^\d$/.test(input.payload) ? "TYPE" : "NONE"),
      payload: input.payload,
    })),
    http$
      .select("node")
      .map((response$) => {
        const no = response$.request.url
          .replace(urlPrefix, "")
          .replace(urlPostfix, "");
...
function model(action$, changeLang$) {
  ...
  const clearReducer$ = action$
    ...
    .map(
      (action) =>
        function clearReducer(oldState) {
  ...
  const deleteReducer$ = action$
    ...
    .map(
      (action) =>
        function deleteReducer(oldState) {
          return {
  ...
  const getDataReducer$ = action$
    ...
    .map(
      (action) =>
        function getDataReducer(oldState) {
          if (action.payload && +action.payload.nid === +oldState.no) {
            if (oldState.auto) {
              ...
              const tip =
                oldState.lang === "en"
                  ? getFieldData(action.payload, "field_title_en", "value")
  ...
  const changeLangReducer$ = changeLang$.map((lang) => (oldState) => {
  ...
  return state$.map(({ no, tip, res, lang }) => {
    ...
    if (no) {
      ...
      if (!tip) {
        c.push(
          span(".tip", [
            res ? text(lang, "No Result!") : text(lang, "Searching..."),
          ])
        );
      }
    }
